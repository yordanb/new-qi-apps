  ...
  loginWithEmail({
    ...
    try {
      ...
      */
      ...
      String role = await getRole(userToken!);
  ...
  login({
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('http://$apiIP:$apiPort/api/login'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        // Redirect to another page with token
        DBService.set("token", token);
        DBService.set("expired_at",
            DateTime.now().add(const Duration(seconds: 10)).toString());
        /*
        isAdmin = responseData['role'] == 'Admin';
        Atasan = responseData['role'] == 'Atasan';
        Bawahan = responseData['role'] == 'Bawahan';
