...
class _PageConfigState extends State<PageConfig> {
  ...
  String _buildApiUrlMP() {
    return "http://$apiIP:$apiPort/api/mp";
  }

  String _buildApiUrlUser() {
    return "http://$apiIP:$apiPort/api/user";
  }
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                await _addDataUser(nameController.text, nrpController.text,
                    crewController.text, posisiController.text);
                Navigator.of(context).pop();
              },
            ),
          ],
  ...
  void _filterDataMP(String query) {
    setState(() {
      _filteredData = _data
          .where((item) => item['nrp']
              .toString()
      ...
      appBar: AppBar(
        title: _isSearching
            ? TextField(
                ...
                onChanged: (query) => _filterDataMP(query),
              )
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: fetchDataMP,
                    child: const Text('MP'),
                  ),
                ),
              ),
              const SizedBox(width: 20), // Spacer antar tombol
              Expanded(
                child: SizedBox(
                  height: 30, // Tinggi tombol
                  child: ElevatedButton(
